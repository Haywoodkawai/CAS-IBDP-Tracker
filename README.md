...
1. Open the link above. To look around before entering anything, press **See an example first**; press
   **Exit example** when you are done. Then set your name, CAS dates and IBDP end date. All of them can be changed later
...
- **The full set of files.** On this page press the green **Code** button, then **Download ZIP**, and unzip it
  ([direct link](https://github.com/haywoodkawai/CAS-IBDP-Tracker/archive/refs/heads/main.zip)).
...
If you change `index.html`, also change `VERSION` in `sw.js` (for example from `cas-tracker-v9` to `cas-tracker-v10`) so devices that already saved the old copy fetch the new one. Saved data is not affected.
...
| `screenshot.png`, `screenshot-first-open.png` | The two pictures shown in this README |
